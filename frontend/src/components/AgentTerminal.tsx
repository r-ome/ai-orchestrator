...
import { agentSocketUrl, type CodingAgent } from '../api/agents'

/** `connecting` until the server's `ready` frame, then `live` until the socket
 *  closes for any reason. */
export type TerminalPhase = 'connecting' | 'live' | 'closed'
...
/** Why these codes: the backend closes with 4404 when the agent is gone, 4409
 *  when another terminal already holds it, and 4503 when Docker is down. */
const CLOSE_REASONS: Record<number, string> = {
  4404: 'This agent no longer exists.',
  4409: 'This agent already has a terminal open somewhere else.',
...
function AgentTerminal({ agent, onPhase, onError }: AgentTerminalProps) {
  ...
  useEffect(() => {
    ...
    // When the active buffer has no scrollback — the alternate screen an agent
    // TUI runs in — xterm converts wheel events into cursor-key sequences. The
    // agent reads those arrows as prompt-history navigation, so the wheel
    // replays old prompts instead of scrolling. Suppress that conversion, but
    // still pass the wheel through when the TUI tracks the mouse itself.
    ...
    const openTimer = window.setTimeout(() => {
      if (cancelled) return
      socket = new WebSocket(agentSocketUrl(agent))
      ...
      socket.onmessage = (event) => {
        ...
        if (control.type === 'ready') {
          ...
          terminal.write(
            `\r\n\x1b[2m— agent exited${code === null || code === undefined ? '' : ` (code ${code})`} —\x1b[0m\r\n`,
          )
      ...
      socket.onclose = (event) => {
        ...
        if (reason) handlersRef.current.onError(reason)
          ...
          handlersRef.current.onError(
            event.reason || `Terminal connection closed (code ${event.code}).`,
          )
        }
      ...
      socket.onerror = () => {
        if (!cancelled) {
          handlersRef.current.onError('Terminal connection failed.')
        }
      }
    }, 0)
    ...
    // The parent changes the component key when the user requests a reconnect.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [socketId])

  return <div className="agent-terminal" ref={hostRef} />
}
